...
class CVFilter:
    ...
    def Filter_state_covariance(self, measurements, current_time):
        ...
        return predicted_range, predicted_azimuth, predicted_elevation, predicted_time, most_likely_measurement
...
# Process measurements and collect data for plotting
for measurement in measurements:
    pred_rng, pred_az, pred_el, pred_time, most_likely_measurement = kalman_filter.Filter_state_covariance([measurement], measurement[3])
    ...
    predicted_range.append(pred_rng)
    predicted_azimuth.append(pred_az)
    predicted_elevation.append(pred_el)
    predicted_time.append(pred_time)

# Plotting
plt.figure(figsize=(15, 10))

# Plot measurement range vs. measurement time
plt.subplot(3, 1, 1)
plt.plot(measured_time, measured_range, 'b', label='Measured Range')
plt.plot(predicted_time, predicted_range, 'r--', label='Predicted Range')
plt.xlabel('Time')
plt.ylabel('Range')
plt.title('Measurement Range vs. Time')
plt.legend()

# Plot measurement azimuth vs. measurement time
plt.subplot(3, 1, 2)
plt.plot(measured_time, measured_azimuth, 'b', label='Measured Azimuth')
plt.plot(predicted_time, predicted_azimuth, 'r--', label='Predicted Azimuth')
plt.xlabel('Time')
plt.ylabel('Azimuth')
plt.title('Measurement Azimuth vs. Time')
plt.legend()

# Plot measurement elevation vs. measurement time
plt.subplot(3, 1, 3)
plt.plot(measured_time, measured_elevation, 'b', label='Measured Elevation')
plt.plot(predicted_time, predicted_elevation, 'r--', label='Predicted Elevation')
plt.xlabel('Time')
plt.ylabel('Elevation')
plt.title('Measurement Elevation vs. Time')
plt.legend()
